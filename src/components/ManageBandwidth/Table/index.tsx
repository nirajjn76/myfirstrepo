...
const PortsTable: React.FC<PortsTableProps> = ({
  ports, groups, loading, order, orderBy, selectedUnGroupId, selected, unGroupedSelected, expandedGroupIds, removeLoadingId, onSaleClick, onRemoveClick, onSaleEdit, onSelectRow, onGroupRowSelect, onRequestSort, onCollapsibleGroupOpen, onCollapsibleGroupClose, onEditGroupClick,
}) => {
  ...
  if (loading) {
    ...
    const portRows = ports.map((port: any, index) => {
      return <PortRow onSaleClick={onSaleClick} onSaleEdit={onSaleEdit} onRemoveClick={onRemoveClick} removeLoadingId={removeLoadingId} port={port} key={index} rowSelected={!!(selected.find((item) => (item.nrId == port.port_id) || (item.purchasePortId == port.purchasePortId)))} selected={selected} onSelectRow={onSelectRow} />;
    });

    const groupRows = groups.map((group: any, index) => {
      return <GroupRow onSaleClick={onSaleClick} onSaleEdit={onSaleEdit} onRemoveClick={onRemoveClick} removeLoadingId={removeLoadingId} open={!!expandedGroupIds.includes(group.groupId)} group={group} selectedUnGroupId={selectedUnGroupId} selected={unGroupedSelected} key={index} colSpan={headCells.length + 1} onCollapsibleGroupClose={onCollapsibleGroupClose} onCollapsibleGroupOpen={onCollapsibleGroupOpen} onGroupRowSelect={onGroupRowSelect} onEditGroupClick={onEditGroupClick} />;
    });
    rows = (
      <TableBody>
        {groupRows}
