...
const GroupTable: React.FC<GroupTableProps> = ({
  open, group, colSpan, selected, selectedUnGroupId, removeLoadingId, onSaleClick, onSaleEdit, onRemoveClick, onCollapsibleGroupClose, onCollapsibleGroupOpen, onGroupRowSelect, onEditGroupClick,
}) => {
  const groupPortRows = group.ports.map((port: any, index: number) => {
    return <GroupPortRow onSaleClick={onSaleClick} key={index} port={port} groupId={group.groupId} removeLoadingId={removeLoadingId} onSaleEdit={onSaleEdit} onRemoveClick={onRemoveClick} selectedUnGroupId={selectedUnGroupId} rowSelected={!!(selected.find((item) => (item.nrId == port.port_id) || (item.purchasePortId == port.purchasePortId)))} selected={selected} onSelectRow={(ports) => onGroupRowSelect(ports, group.groupId)} />;
  });
