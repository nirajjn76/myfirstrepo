...
const GroupDrawer: React.FC<GroupDrawerProps> = ({
  ...
  const disabled = useMemo(() => {
    ...
    if (selectedExistingGroup && selectedExistingGroup !== -1) {
      resp = false;
    } else if (!groupName || !groupDescription) {
      resp = true;
    }
    return resp;
  }, [selectedExistingGroup, groupName, groupDescription]);
  ...
  const handleAddGroupClick = useCallback(() => {
    ...
    if (selectedExistingGroup && selectedExistingGroup !== -1) {
      ...
      ManageBandwidthService.addNrsInExistingGroup(payload)
        .then(() => {
      ...
      ManageBandwidthService.addGroupDetails(payload)
        .then(() => {
          toast.success(SuccessMessage.addGroupSuccess, { icon: <img src={SuccessInfoIcon} alt="Success" /> });
          setLoading(false);
          setGroupNameError('');
  ...
  }, [setLoading, onClose, setGroupNameError, groupedSelected, groupName, groupDescription, selectedExistingGroup, expandedNeId]);
