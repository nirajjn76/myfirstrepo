...
describe('<ManageBandwidth />', () => {
  ...
  test('Port view switch is available', async () => {
    ...
    await waitFor(() => {
      const portView = container.baseElement.querySelector('.show-verified-div');
      expect(portView?.firstChild?.textContent?.includes('Port View')).toBeTruthy();
    });
  });
});
